...
@click.option(
    "-o", "--output", "output_path", default="./output/temp/docs.json", help="输出JSON路径"
)
...
@click.option(
    "-o", "--output", "output_path", default="./output/temp/train.jsonl", help="输出JSONL路径"
)
...
@click.option("--data", "data_path", default="./output/temp/train.jsonl", help="训练数据路径")
...
@cli.command()
@click.option(
    "--doc-dir", "doc_dir", required=True, help="文档目录 (支持PDF/DOCX/TXT/MD)"
)
@click.option("--model", "model_name", default="Qwen2.5-0.5B", help="模型名称")
@click.option(
    "--model-path", "model_path", default=None, help="本地模型路径(优先于model_name)"
